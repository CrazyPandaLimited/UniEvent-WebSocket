#include <xs/websocket/server/error.h>
#include <typeinfo>
#include <cxxabi.h>
...
SV* xs::websocket::server::error_sv (const Error& err, bool with_mess) {
    dTHX;
    int status;
    ...
    if (strstr(class_name, "panda::websocket::") == class_name) sv_catpv(stash_name, class_name + 18);
    free(class_name);
